...
import com.example.notesapp.features.note.presentation.notes.components.NoteItem
import com.example.notesapp.features.note.presentation.notes.components.OrderSection
    ...
    Scaffold(scaffoldState = scaffoldState, floatingActionButton = {
        ...
        Column(
            ...
            Row(
                ...
                IconButton(onClick = {
                    viewModel.onEvent(NotesUserEvent.Toggle)
                }) {
                    ...
                    NoteItem(note = note, modifier = Modifier
                        ...
                        .clickable {
                            navController.navigate(
                                Screen.AddEditNoteScreen.route + "?$PARAM_NOTE_ID=${note.id}&$PARAM_NOTE_COLOR=${note.color}"
                            )
                        }) {
                        viewModel.onEvent(NotesUserEvent.DeleteNote(note))
                        scope.launch {
                            val result = scaffoldState.snackbarHostState.showSnackbar(
                                message = "Note delete", actionLabel = "Undo"
                            )
                            if (result == SnackbarResult.ActionPerformed) viewModel.onEvent(
                                NotesUserEvent.RestoreNote
                            )
                        }
                    }
